...
public function __construct() {
 ...
 public function insertPost($email, $title, $newspost){
    $sql = 'INSERT INTO post SET unique_id =:unique_id,title =:title,newspost =:newspost,created_at = NOW()';
    $query = $this -> conn ->prepare($sql);
    $uniqid = $this -> getUniqueId($email);
    $query->execute(array(':unique_id' => $uniqid, ':title' => $title, ':newspost' => $newspost));
    if($query) {
      return true;
    } else {
      return false;
...
public function getListPosts($email){
  $sql = 'SELECT title, newspost, created_at from post WHERE unique_id =:unique_id';
  $query = $this -> conn -> prepare($sql);
  $uniqid = $this -> getUniqueId($email);
  $query ->execute(array(':unique_id' => $uniqid));
  $datas = array();
